...
void IOgen_loc(int tasknum, rttask* tasks,int runtime, int offset){
    //generates a set of target address for write or read workload.
    //Address is generated according to the task parameter and target runtime.
    FILE* write_fp = fopen("workload_w.csv","w");
    FILE* read_fp = fopen("workload_r.csv", "w");
    ...
    //FIXME::write workload generates from index 1 to total_w+1, due to the locality gen
    for(int i=1;i<total_w+1;i++){
        //simple locality change workload generation
        hot = (float)(rand()%100/100.0);
        if (total_w <= 20*i && total_w >= 5*i){
            if (hot < TPLOCAL){
                lpa = rand()%hotspace + hotspace;//offset can be given
            }
            else {
                lpa = rand()%(logispace);
                while(lpa <= hotspace+hotspace && lpa >= hotspace){
                    lpa = rand()%(logispace);
                }
            }
        }
