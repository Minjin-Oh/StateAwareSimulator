...
for U in "${UTILS[@]}"; do
    ...
    for i in $(seq 1 "$NUM_RUNS"); do
        ...
        ( cd "$WD_BASELINE" && taskset -c 9  "$BIN" NO     NO        SKIPRR  nogen 4 "$U" -1 0.05 0.95 0 -1 1.0 > /dev/null 2> run.err ) &
        pid1=$!
        ( cd "$WD_DYNAMIC"  && taskset -c 10 "$BIN" NO     MOTIVALLY SKIPRR  nogen 4 "$U" -1 0.05 0.95 0 -1 1.0 > /dev/null 2> run.err ) &
        pid2=$!
        ( cd "$WD_STATIC"   && taskset -c 11 "$BIN" NO     NO        BASE005 nogen 4 "$U" -1 0.05 0.95 0 -1 1.0 > /dev/null 2> run.err ) &
        pid3=$!
        ( cd "$WD_HYBRID"   && taskset -c 12 "$BIN" NO     MOTIVALLY BASE005 nogen 4 "$U" -1 0.05 0.95 0 -1 1.0 > /dev/null 2> run.err ) &
        pid4=$!
        ( cd "$WD_WONLY"    && taskset -c 13 "$BIN" NO     INVW      SKIPRR  nogen 4 "$U" -1 0.05 0.95 0 -1 1.0 > /dev/null 2> run.err ) &
        pid5=$!
        ( cd "$WD_LAWLD"    && taskset -c 14 "$BIN" UTILGC INVW      SKIPRR  nogen 4 "$U" -1 0.05 0.95 0 -1 1.0 > /dev/null 2> run.err ) &
        pid6=$!
        ( cd "$WD_LAWL"     && taskset -c 15 "$BIN" UTILGC INVW      RR005   nogen 4 "$U" -1 0.05 0.95 0 -1 1.0 > /dev/null 2> run.err ) &
        pid7=$!
