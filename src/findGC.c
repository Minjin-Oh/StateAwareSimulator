...
int _find_gc_safe(rttask* tasks, int tasknum, meta* metadata, int old, int taskidx, int type, float util, int cur_b, int rsv_b){
    ...
    float r_exec_rsv_b = r_exec(rsv_b_state);
...
block* find_gc_utilsort(rttask* task, int taskidx, int tasknum, meta* metadata, bhead* full_head, bhead* rsvlist_head, bhead* write_head, FILE* gc_detail){
    ...
    // int best_safe_idx = -1;                         // best victim passing schedulability
    // int best_fallback_idx = -1;                     // best MINRC-only victim
    block* best_safe_blk = NULL;                         // best victim passing schedulability
    block* best_fallback_blk = NULL;                     // best MINRC-only victim
    float best_safe_util = 2.0f;                    // minimum util among schedulable victims
    float best_fallback_util = 2.0f;                // minimum util among MINRC victims
    ...
    // opt-ver.
    // 1. Scan blocks once and keep
    //    a) the best schedulable victim (best_safe_blk) and
    //    b) the best fallback victim satisfying only MINRC (best_fallback_blk)
    block* cur = full_head->head;
    while(cur != NULL){
        cur_state = metadata->state[cur->idx];
        ...
        if(new_rc >= MINRC){
            ...
            // Always update MINRC-only fallback candidate
            if(gc_util < best_fallback_util){
                best_fallback_util = gc_util;
                // best_fallback_idx = cur->idx;
                best_fallback_blk = cur;
            }

            // check util constraint
            if(_find_gc_safe(task,tasknum,metadata,old,taskidx,GC,gc_util,cur->idx,rsvlist_head->head->idx) == 0){
                if(gc_util < best_safe_util){
                    best_safe_util = gc_util;
                    // best_safe_idx = cur->idx;
                    best_safe_blk = cur;
                }
            }
        }
    ...
    // 2. EDGE CASE HANDLING: if no schedulable victim, use MINRC-only fallback
    // if(best_safe_idx == -1 && best_fallback_idx != -1){
    //     printf("vic_num == 0\n");
    //     return best_fallback_idx;
    // }
    // return best_safe_idx;
    if(best_safe_blk == NULL && best_fallback_blk != NULL){
        printf("vic_num == 0\n");
        return best_fallback_blk;
    } else {
        return best_safe_blk;
    }
}
